...
Template.tokenForm.events({
  ...
  'click .check'(event, template) {
    ...
    if (theAddress) {
      ...
      if (web3.utils.isAddress(Session.get('contractAddress').toUpperCase())) {
        ...
        Meteor.call('getCode', theAddress, function(err, res) {

          console.log(theAddress, err, res);
                  ...
                  if (tokenContract) {
                    tokenContract.name().call().then(function (resp) {
                      if (resp) {
                        ...
                        Meteor.call('saveFile', res, Session.get('contractAddress') + '.byte')
  ...
  'submit .tokenForm'(event, template) {
